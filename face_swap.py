...
COLOUR_CORRECT_BLUR_FRAC_im1 = 0.4  # src image
COLOUR_CORRECT_BLUR_FRAC_im2 = 0.6  # dst image
...
def correct_colours(im1, im2, landmarks1):
    blur_amount_im1 = COLOUR_CORRECT_BLUR_FRAC_im1 * np.linalg.norm(
        np.mean(landmarks1[LEFT_EYE_POINTS], axis=0) -
        np.mean(landmarks1[RIGHT_EYE_POINTS], axis=0))
    blur_amount_im2 = COLOUR_CORRECT_BLUR_FRAC_im2 * np.linalg.norm(
        np.mean(landmarks1[LEFT_EYE_POINTS], axis=0) -
        np.mean(landmarks1[RIGHT_EYE_POINTS], axis=0))

    blur_amount_im1 = int(blur_amount_im1)
    if blur_amount_im1 % 2 == 0:
        blur_amount_im1 += 1
    blur_amount_im2 = int(blur_amount_im2)
    if blur_amount_im2 % 2 == 0:
        blur_amount_im2 += 1

    im1_blur = cv2.GaussianBlur(im1, (blur_amount_im2, blur_amount_im2), 0)
    im2_blur = cv2.GaussianBlur(im2, (blur_amount_im1, blur_amount_im1), 0)
...
def face_swap(src_face, dst_face, src_points, dst_points, dst_shape, dst_img, correct_color):
    h, w = dst_face.shape[:2]

    # 3d warp
    warped_src_face = warp_image_3d(src_face, src_points[WRAP_REGION], dst_points[WRAP_REGION], (h, w))
